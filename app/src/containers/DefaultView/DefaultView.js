...
class DefaultView extends Component {
  ...
  editPostHandler = (id, category) => {
    this.props.history.push(`/${category}/edit/${id}`);
  };

  selectedPostHandler = (id, category) => {
  ...
  render() {
    ...
    return (
      <div className="default-view">
        <Controls
          sortByVoteHandler={this.sortByVoteHandler}
          sortByTimeHandler={this.sortByTimeHandler}
          addHandler={this.addPostHandler}
          numericAsc={this.state.ascVoteOrder}
        />
        <div className="row">
          <div className="col-12 col-md-4 col-lg-4">
            <Categories
              items={this.props.categories}
              onCategorySelected={this.getPostByCategory}
            />
          </div>
          <div className="col-12 col-md-8 col-lg-8">{posts}</div>
        </div>
      </div>
    );
  }
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    sortByTimestamp: (posts, order) =>
      dispatch(actions.sortByTimestamp(posts, order)),
    sortByVote: (posts, order) => dispatch(actions.sortByVotes(posts, order)),
    getPostByCategory: category =>
      dispatch(actions.getPostByCategory(category)),
